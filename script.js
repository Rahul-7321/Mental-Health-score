(() => {
  ...
  // ---------------------------------------------------------
  // Maps between the form's internal (snake_case) field keys
  // and the exact field names the FastAPI StudentData model
  // expects. These MUST match the Pydantic model in app.py.
  // ---------------------------------------------------------
  ...
  const API_TO_FORM_FIELD = Object.fromEntries(
    Object.entries(FORM_TO_API_FIELD).map(([formKey, apiKey]) => [apiKey, formKey])
  );

  function toApiPayload(payload) {
    const apiPayload = {};
    Object.entries(FORM_TO_API_FIELD).forEach(([formKey, apiKey]) => {
      apiPayload[apiKey] = payload[formKey];
    });
    return apiPayload;
  }
  ...
  // ---------------------------------------------------------
  // Gather form data into the internal (snake_case) shape.
  // This is converted to the API's exact field names via
  // toApiPayload() right before sending.
  // ---------------------------------------------------------
  ...
  // field-level messages where possible. Server error `loc`
  // values use the API's PascalCase field names (e.g.
  // "Stress_Level"), so they're mapped back to the form's
  // element ids before we can highlight the right input.
  // ---------------------------------------------------------
  function applyServerValidationErrors(detail) {
    ...
    detail.forEach((err) => {
      ...
      const target = formField === "stress_level" ? stressHiddenInput : (formField ? document.getElementById(formField) : null);
      if (target) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      renderError(
        ...
        `Couldn't connect to ${API_BASE}. Make sure the backend is running (uvicorn app:app --port 8000 --reload) and reachable from this page.`
      );
    } finally {
      setSubmitting(false);
    }
